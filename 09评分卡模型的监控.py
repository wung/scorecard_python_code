...
def score_psi(df1,df2,id_col,score_col,x,y,step=None):
    ...
    model_score_group = df1.groupby('score_bin',as_index=False)[id_col].count(). assign(pct=lambda x:x[id_col]/x[id_col].sum()).\
        rename(columns={id_col:'建模样本户数',\
                                           'pct':'建模户数占比'})
    df2['score_bin'] = pd.cut(df2[score_col],bins=np.arange(x,y,step))
    online_score_group = df2.groupby('score_bin',as_index=False)[id_col].count().assign(pct=lambda x:x[id_col]/x[id_col].sum()).\                           
    rename(columns={id_col:'线上样本户数',\
        'pct':'线上户数占比'})
...
def var_stable(score_result,df,var,id_col,score_col,bins):
    ...
    model_var_group = score_result.loc[score_result.col==var,\
        ['bin','total','totalrate','score']].reset_index(drop=True).rename(columns={'total':'建模用户数',\
                                      'totalrate':'建模用户占比','score':'得分'})
    df['bin'] = pd.cut(df[score_col],bins=bins)
    online_var_group = df.groupby('bin',as_index=False)[id_col].count() .assign(pct=lambda x:x[id_col]/x[id_col].sum())\
